<template>
  <div class="wrapper no-print">
    ...
    <el-dialog
      ...
      :modal='false'>
      <el-form :model="formInfo" :rules="rules" ref="formInfo" label-width="100px" class="demo-formInfo">
...
export default {
  ...
  methods: {
    // 鼠标移入菜单时
    menuEnter(name) {
      if (name === '分析研判' || name === '智能预案' || name === '防汛信息管理') {
    ...
    goRouter(router, name, type) {
      if (name === '分析研判' || name === '智能预案' || name === '防汛信息管理' || type == 0) return
...
<style scoped lang="less">
  ...
  /deep/ .el-dialog__header {
    background: linear-gradient(left, rgba(255, 255, 255, 0.4), rgba(5, 56, 87, 0));
