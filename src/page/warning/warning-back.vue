          ...
          <div class="swiper-wrapper">
            <div class="swiper-slide">
              <component
                ...
                :is="nameList[8] && nameList[8].url"
              ></component>
              <component
                :class="className(nameList[12] && nameList[12].name)"
                @weatherMap="changeWeatherMap"
                @treeCheckedCamera="treeCheckedCamera"
                @isShow="setIsShow"
                @changeIndex="changeIndex"
                ...
                :bukongLength="bukongLength"
                :wurenjiLength="wurenjiLength"
                :ref="nameList[12] && nameList[12].name"
                :is="nameList[12] && nameList[12].url"
              ></component>
              <component
...
import Cheliangguiji from "./modules/cheliangguiji/cheliangguiji"
...
export default {
  ...
  computed: {
    ...
    //   })
    // },
    ...mapGetters("warning", ["collapse", "waterState"]),
  },
  mixins: [loading],
  mounted() {
    ...
    getJCYJMenuOrder().then((res) => {
      res.push({id:"ceb0fa4ecfa2978658c3bf5492f3c213",orderNum :12,type: "监测预警",vueUrl:"cheliangguiji"});
      this.nameList = res.map((item, index) => {
        // if (item.vueUrl === 'yuqingjiance') {
        //   item.orderNum > 6 ? this.detailClass = 'right' : this.detailClass = 'left'
        // }
        if (
          item.vueUrl === "shipingjiankon" || item.vueUrl === "cheliangguiji"
        ) {
          return {
            url: () =>
              import("./modules/" + item.vueUrl + "/" + item.vueUrl + ".vue"),
            name: item.vueUrl,
          };
        }
      });
      ...
      if (
        this.nameList[3] !== null ||
        this.nameList[4] !== null ||
        this.nameList[5] !== null
      ) {
        this.addSwiper(".swiper-container", 0);
      }
      if (
        this.nameList[9] !== null ||
        this.nameList[10] !== null ||
        this.nameList[11] !== null
      ) {
  ...
  methods: {
    ...
    changeWeatherMap(arr) {
      ...
      if (index === 1 || index === 2) {
        if (bool) {
          ...
          if (index === 1) {
            url =
              "http://data.istrongcloud.com/data/images/cloud/szmb_transparent.json";
            type = "卫星云图";
          } else if (index === 2) {
            // url = 'http://data.istrongcloud.com/data/images/radar/mingle/caiyun_transparent.json'
            url =
              "https://api.caiyunapp.com/v1/radar/fine_images?lon=120.94146&lat=27.999399&level=1&token=Y2FpeXVuIGFuZHJpb2QgYXBp";
            type = "气象雷达";
          }
    ...
    ...mapActions("warning", ["SetMapLoaded", "SetCurrentWaterState"]),
  },
  beforeDestroy() {
...
/deep/ .swiper-container-android .swiper-slide,
.swiper-wrapper {
