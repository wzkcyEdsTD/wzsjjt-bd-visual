<template>
  <div class='dizai-dialog' :class="isShow?'active':''">
    ...
    <ul class='dizai-tab'>
      <li
        v-for='(value, index) of tabs[dizaiData.type]'
        :key='index'
...
export default {
  ...
  methods: {
    ...
    drawEchart(data) {
      ...
      const unit = this.tabs[this.dizaiData.type].unit
      const res = []
      data.map(item => {
      ...
      this.mychart.setOption({
        ...
        series: [
          {
            name: this.tabs[this.dizaiData.type].unit,
            color: '#FFA533',
            symbolSize: 10,
            data: res,
            type: 'line',
    ...
    downloadImg() {
      ...
      //  对下载的文件命名
      link.download =
        this.tabs[this.dizaiData.type][this.tabIndex] + '图表.png'
    ...
    queryData() {
      ...
      console.log(startTime)
      getDeviceDetail(this.tabIndex, this.dizaiData.deviceId, startTime, endTime, this.dizaiData.tablename, this.startTime || this.endTime ? 0 : this.timeRadio).then(res => {
        console.log(res)
        this.drawEchart(res)
      })
    }
