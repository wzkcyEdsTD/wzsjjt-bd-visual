...
<script>
import { getWZTimeRainInfo, getWZDistructRainInfo } from 'api/warning/warning'
...
export default {
  ...
  methods: {
    ...
    rowClick(data) {
      data.alias = data.alias + '-wenzhou'
      this.SetSpecalPoint(data)
      const parent = this.$parent.$refs['yuqingjiance-wenzhou']
      if (parent._data.dotIndex[parent._data.btnIndex] === 0) {
        parent.changeDot()
      }
    },
    close() {
    ...
    confirm() {
      this.loading2 = true
      getWZTimeRainInfo(this.value, this.districtName, this.type).then(res => {
        this.loading2 = false
        this.table2 = res
      })
    },
    ...
    queryData() {
      ...
      getWZDistructRainInfo(this.value, this.type).then(res => {
        this.loading1 = false
        this.table1 = res
        if (this.$store.state.userInfo.districtName !== '文成县') return
        const dataName = []
