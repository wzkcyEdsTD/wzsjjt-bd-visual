<template>
  <div class="wrapper">
    <div class="video-block">
      ...
      <div><span class="konjian-search" @click="spaceSearch" :class="{active: searchActive}">空间查询</span></div>
      <div class="video-buttons">
        <el-select v-model="xqValue" @change="getVideoData" :popper-append-to-body="appendBody">
      ...
      <nav>
        <span v-for='(item, index) in navList' @click="changeId(item)" :class="{active: item.id === videoListId}" :key='"n" + index'>{{item.name}}</span>
      </nav>
      <div
      ...
      >
        ...
        <el-tree
          ...
          <span class="custom-tree-node" slot-scope="{ node, data }">
            ...
            <span v-show="data.level === 1" @click="showInMap($event, data.id, data.icon)" class="dot" :class="{'active': data.isActive}"> </span>
            <span @click="changeCollection(data.collection, data.leaf, data.id, $event)" :class="{'sign': typeof data.collection === 'boolean', 'active': data.collection}" > </span>
          </span>
        </el-tree>
      </div>
  ...
  methods: {
    ...
    loadNode(node, resolve) {
      ...
      if (node.data && node.data.count > 0) {
        ...
        if (this.$store.state.userInfo.district !== '3303') requestId = node.data.id
        getVideoTreeDetail(this.smidList ? '' : (this.xqValue || node.data.code), requestId, this.shoucangShow, this.searchContent, this.smidList).then(res => {
          res = res.map(val => {
