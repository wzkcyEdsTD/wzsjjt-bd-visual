  ...
  <div class="steps" v-if="isShow">
    <div class="lb-box">
      ...
      <ol class="lb-sign flex" @mouseenter="mouseChange(true)" @mouseleave="mouseChange(false)">
        <li v-for="(item,index1) in list" :key="index1" class="flex-1">
          <dl>
      ...
      <!-- 轮播控件 -->
      <div class="lb-ctrl left" @mouseenter="mouseChange(true)" @mouseleave="mouseChange(false)" @click="changeIndex(-1)">＜</div>
      <div class="lb-ctrl right" @mouseenter="mouseChange(true)" @mouseleave="mouseChange(false)" @click="changeIndex(1)">＞</div>
...
export default {
  ...
  mounted() {
    this.$bus.$on('rainMap', (data) => {
      console.log(data)
      if (data.length) {
        this.index = 0
        this.isShow = true
        if (this.$parent.$refs.qixiangxinxi) {
          this.$parent.$refs.qixiangxinxi.clickRainfall(data[0].num, data[0].value)
        }
        this.list = data
        this.startActive()
      } else {
  ...
  methods: {
    ...
    changeActive(index) {
      this.index = index
      this.$parent.$refs.qixiangxinxi.clickRainfall(this.list[this.index].num, this.list[this.index].value)
    },
    changeIndex(index) {
