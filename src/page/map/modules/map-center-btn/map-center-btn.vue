<template>
  <div class="wrapper">
    ...
    <div class="toCenter1" :class="collapse1?'collapse':''" title="全图">
      <i style="width: 100%;height: 0.42rem;" @click="changeMapTollBar({ name: '地图居中', value: 'map_init' })"></i>
    </div>
    ...
    <div class="map-toolbar-box-map" :class="{'collapse': collapse1, active: toolShow}">
      <span class="collapse-btn" :class="{active: toolShow}" title="地图工具"><i
        style="width: 32px;height: 32px;"
        @click="mapTool"></i></span>
      ...
      <div>
        <div :key="'b'+index2" v-for="(item2,index2) in mapNew[selectIndex].children">
          <label
            ...
            @click="changedituType(item2)"><span></span>{{item2.name}}</label>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    setOpacity(event) {
      ...
      if (this.isSetOpacity) {
        ...
        this.$refs.slider.style.top = client.y +
          (parseInt(window.getComputedStyle(this.$refs.slider).height) / 2) + 'px'
    ...
    pointSearch(event) {
      ...
      if (this.isPointSearch) {
        ...
        const style = window.getComputedStyle(event.currentTarget)
        const l = parseInt(style.marginLeft) + parseInt(style.width)
        this.$refs.longitudeSearch.style.top = client.y +
          (parseInt(window.getComputedStyle(this.$refs.longitudeSearch).height) / 2) + 'px'
    ...
    // 修改地图类型
    changeMapType(item) {
      if (item.value === this.currentMapType) {
        return
      }
      this.canCilckMapType = false
      if (!((this.currentMapType === 'juhe' && item.value === 'sandian') || (this.currentMapType === 'sandian' && item.value === 'juhe'))) {
...
<style scoped lang="less">
  ...
  .map-toolbar-box {
    ...
    .map-type {
      ...
      .item.disabled:hover, .item.disabled {
        color: #999;
        cursor: default;
      }
      .item1 {
