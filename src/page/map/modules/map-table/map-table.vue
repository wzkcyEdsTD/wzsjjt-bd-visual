...
export default {
  ...
  computed: {
    ...
    areaProp() {
      let res = this.currentOnePoint.showField && this.currentOnePoint.showField[0].toLowerCase() === 'district' ? (this.userInfo.district && this.userInfo.district.length > 4 ? 'street' : 'district') : this.currentOnePoint.showField[0].toLowerCase()
      if (this.currentOnePoint.level - 0 === 3 && this.currentOnePoint.name.lastIndexOf('专家') === this.currentOnePoint.name.length - 2) {
        const parentNodeData = this.$parent.$parent.$refs.tree.getTreeNode(this.currentOnePoint.key).parent.data
  ...
  methods: {
    ...
    initData() {
      if (this.currentOnePoint && this.currentOnePoint.datasetname) {
        const showFileds = JSON.parse(JSON.stringify(this.currentOnePoint.showField))
        ...
        likeStatistics(this.search, showFileds, exactQueryKey, this.exactQuery.value, this.exactBarQuery.key, this.exactBarQuery.value, this.pageNo, this.pageSize, this.currentOnePoint.datasetname.split(':')[1], this.userInfo.district, this.currentOnePoint.sql, showFileds[showFileds.length - 1]).then((data) => {
...
<style scoped lang="less">
  ...
  /deep/ .el-table tr:hover, /deep/ .current-row {
    cursor: pointer;
