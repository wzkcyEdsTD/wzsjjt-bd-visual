<template>
  <div class="page-map">
    <div class="map" :class="this.collapse1?'collapse':''">
      <BaseMap
        class="is-print"
        ref='baseMap'
        ...
        :onePoint="onePoint"
        :mapTollBar="mapTollBar"></BaseMap>
      <MapCenterBtn
        ref="mapCenterBtn"
        class="no-print"
        @setMapTollBar="setMapTollBar"
        :currentBaseMapType="currentBaseMapType"></MapCenterBtn>
    </div>
    <MapBox class="box no-print" @changeCollapse="changeCollapse1" :collapse="collapse1" region="west">
      ...
      <div class="tree flex-1">
        <MapTree
          ...
          @checkedItem="treeCheckedItem"></MapTree>
      </div>
    </MapBox>
    <MapBox
      class="box no-print"
        ...
        <div class="details flex flex-y">
          ...
          <div class="flex-1">
            <MapTable
              ...
              @rowClick="tableRowClick"></MapTable>
          </div>
        </div>
      </div>
    </MapBox>
    ...
    <Around
      ...
      @selectChange="aroundSelectChange"></Around>
    <Space class="no-print" ref="space" :data="spaceDataValue" @rowClick="tableRowClick" @close="closeSpace"></Space>
    <MapTableFull
      class="no-print"
      ref="mapTableFull"
      ...
      :count="tableNum"
      @rowClick="tableRowClick"
      @close="changeTitleBtn"></MapTableFull>
    <!--    <i class="print no-print" :class="this.collapse2?'collapse':''" @click="print">-->
    <!--      <span></span>-->
    <!--    </i>-->
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    delItem(data) {
      ...
      // var arr = this.$refs.tree.getTreeCheckKey()

      // var index = arr.indexOf(data.key)
      // let key
      // if (index !== -1) {
      //   key = arr.splice(index, 1)
      // }
      // if (data.pid) {
      //   var pidIndex = arr.indexOf(data.pid)
      //   if (pidIndex !== -1) {
      //     arr.splice(pidIndex, 1)
      //     var ppNode = this.$refs.tree.getTreeNode(data.pid)
      //     if (ppNode) {
      //       var ppidIndex = arr.indexOf(ppNode.data.pid)
      //       if (ppidIndex !== -1) {
      //         key = arr.splice(ppidIndex, 1)
      //       }
      //     }
      //   }
      // }
      // this.$refs.tree.setTreeCheckKey(arr, key)
    },
    // 设置企业个数
    getTableNum(num) {
      this.tableNum = num
  ...
  watch: {
    currentOnePoint() {
      ...
      if (this.currentOnePoint) {
        // 如果当前节点是面
        if (this.currentOnePoint.geotype === 'polygon' && this.currentOnePoint.num === 0) {
          //  并且没有children
          if ((!this.currentOnePoint.children || (this.currentOnePoint.children && this.currentOnePoint.children.length === 0))) {
...
<style scoped lang="less">
  ...
  @media print {
    ...
    /deep/ .is-print {
      ...
      .search-body, .longitudeSearch, .mapboxgl-ctrl-lngLat {
        display: none !important;
      }
      .slider-wrapper, .toCenterRight, .closeMap {
        display: none;
      }
      #base-map {
